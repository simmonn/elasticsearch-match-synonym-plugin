        ...
        private void add(BooleanQuery.Builder q, String field, List<TermAndBoost> current, BooleanClause.Occur operator, boolean isPrefix) {
            ...
            if (current.size() == 1) {
                if (isPrefix) {
                    ...
                    q.add(newTermQuery(current.get(0).term, BoostAttribute.DEFAULT_BOOST), operator);
                }
            } else {
                // We don't apply prefix on synonyms
                final TermAndBoost[] termAndBoosts = current.toArray(new TermAndBoost[0]);
...
/*                        final TermAndBoost[] termAndBoosts = Arrays.stream(terms)
                        ...
                        TermAndBoost[] termAndBoosts = new TermAndBoost[terms.length];
                        for (int j = 0; j < terms.length; j++) {
                            if (j != 0) {
                                boost = synonymBoost;
                            }
                            termAndBoosts[j] = new TermAndBoost(terms[j], boost);
                        }
